...
Release 应用**使用 Developer ID 签名并经 Apple 公证**。首次发布前需要有效 Apple Developer Program 会员、已安装的 `Developer ID Application` 证书，以及 `notarytool` Keychain profile。发布脚本读取本机环境变量 `DEVELOPMENT_TEAM`、`DEVELOPER_IDENTITY` 和 `NOTARY_PROFILE`，不保存任何凭据到仓库。
